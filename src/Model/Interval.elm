...
compare (Interval intA) (Interval intB) =
    if (Date.compare (intA.start) (intB.start) ) == EQ then
        case (intA.end, intB.end) of
            (Just _, Just _) ->
                compareLength (Interval intA) (Interval intB)
            (Just _, Nothing) ->
                LT
            (Nothing, Just _) ->
                GT
...
compareLength : Interval -> Interval -> Order
compareLength intervalA intervalB =
    case (length intervalA, length intervalB) of
        (Just (yearsA, monthsA), Just (yearsB, monthsB)) ->
            if (yearsA, monthsA) > (yearsB, monthsB) then
                GT
            else if (yearsA, monthsA) < (yearsB, monthsB) then
                LT
            else
                EQ
        (_, _) ->
...
view interval =
    let 
        (Interval aux) = interval
        startInterval=p[class "interval-start"][text<|"start: " ++ String.fromInt(Date.year aux.start) ++ maybeMonthToString(Date.month aux.start)]
        endInterval=p[class "interval-end"][text<|"end: " ++ maybeDateToStringText aux.end]
        len=p[class "interval-length"][text<|"length: " ++ lenghtText interval]
    in
    div [class "interval"] 
    [startInterval,
    endInterval,
...
maybeDateToStringText : Maybe Date -> String
maybeDateToStringText maybeEndDate =
    case maybeEndDate of
        Just endDate ->
             String.fromInt(Date.year endDate) ++ maybeMonthToString(Date.month endDate)

        Nothing ->
            "Present"

lenghtText : Interval -> String
lenghtText interval=
