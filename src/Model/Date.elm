...
monthsBetween dA dB =
    let
        monthToInt_ : Maybe Month -> Int
        monthToInt_ maybeMonth =
            Maybe.map monthToInt maybeMonth |> Maybe.withDefault 0
    in
    if month dA == Nothing && month dB == Nothing then
       Just (abs(year dA - year dB)*12)
        ...
        Just (abs((year dA - year dB)*12 + monthToInt_ (month dA) - monthToInt_ (month dB)))
...
compare (Date d1) (Date d2) =
    let
        compYear = Basics.compare d1.year d2.year
    in
    if compYear == EQ then
